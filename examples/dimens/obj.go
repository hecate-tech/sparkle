...
//        2nd coord
// 1st coord
func LoadObj(objFile, texFile string, w, h int, ctx *sparkle.Context) (*Model, error) {
	...
	for scanner.Scan() {
		...
		if line[0] == 'v' && line[1] == 'n' {
			...
			mod.outElements = append(mod.outElements, vertexIndex[0])
			mod.outElements = append(mod.outElements, uvIndex[0])
			mod.outElements = append(mod.outElements, normalIndex[0])

			mod.outElements = append(mod.outElements, vertexIndex[1])
			mod.outElements = append(mod.outElements, uvIndex[1])
			mod.outElements = append(mod.outElements, normalIndex[1])

			mod.outElements = append(mod.outElements, vertexIndex[2])
			mod.outElements = append(mod.outElements, uvIndex[2])
			mod.outElements = append(mod.outElements, normalIndex[2])

			// vertices[ib].Sub(vertices[ia]).Cross(
			// 	 vertices[ic].Sub(vertices[ia]),
			// ).Normalize()
...
func (m *Model) processNode() {
	for i := uint(0); i < m.numMeshes; i++ {

	}
}

// func (m *Model) processMesh() Mesh {
// 	var vertices []mgl64.Vec3
// 	var indices []float32
// 	var textures []gl.Texture

// 	for _, v := range m.outVertices {
